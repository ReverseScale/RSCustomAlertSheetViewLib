...
@interface RSActionSheet()<UITableViewDelegate,UITableViewDataSource>
@property (nonatomic,strong) UITableView *sheetTable;
@property (nonatomic,assign) RSActionSheetStyle actionsheetStyle;
@end
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    RSActionSheetCell *cell = [tableView dequeueReusableCellWithIdentifier:identifier];
    if (!cell) {
        cell = [[RSActionSheetCell alloc] initWithStyle:(UITableViewCellStyleDefault) reuseIdentifier:identifier];
    }
    
    if (_actionsheetStyle == RSActionSheetDefault) {
        [cell setupRSActionSheetDefaultCellWithTitle:_sheetTitleArr[indexPath.row] CellHeight:_cellHeight];
    }else if (_actionsheetStyle == RSActionSheetIconAndTitle){
        UIFont *font = _subtitleFont?_subtitleFont:[UIFont systemFontOfSize:17];
        [cell setupRSActionSheetIconAndTitleWithTitle:_sheetTitleArr[indexPath.row] titleFont:font icon:_iconArr[indexPath.row] cellHeight:_cellHeight];
    }else if (_actionsheetStyle == RSActionSheetIcon){
        [cell setupRSActionSheetIconAndTitleWithIcon:_iconArr[indexPath.row] cellHeight:_cellHeight];
    }
...
#pragma mark RSActionSheetDelegate
